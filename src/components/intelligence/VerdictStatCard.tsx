...
function ApprovalBadge({ pct }: { pct: number }) {
  const healthy = pct >= 70
  const warn    = pct >= 40
  return (
    <span className="flex items-center gap-1.5 text-sm font-medium">
      <span className={cn('w-2 h-2 rounded-full',
        healthy ? 'bg-emerald-50 dark:bg-emerald-950/400' : warn ? 'bg-amber-400' : 'bg-red-400')} />
      <span className={healthy ? 'text-emerald-600 dark:text-emerald-400' : warn ? 'text-amber-500 dark:text-amber-400' : 'text-red-500 dark:text-red-400'}>
        {pct}% approval
      </span>
    </span>
  )
...
function ApprovalPill({ pct }: { pct: number }) {
  ...
  return (
    <div className="rounded-lg bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 px-3 py-2.5 flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        <span className="text-xs font-normal text-stone-600 dark:text-slate-400">Approval rate across all verdicts</span>
        <span className="text-xs font-semibold text-gray-400 dark:text-slate-500">{pct}%</span>
      </div>
      <div className="w-full bg-gray-100 dark:bg-neutral-800 rounded-full h-1.5">
        <div className={cn('h-1.5 rounded-full transition-all', barColor)} style={{ width: `${capped}%` }} />
      </div>
    </div>
  )
}

export function VerdictStatCard({ stat }: Props) {
  const meta  = AGENT_META[stat.critic_id] ?? { icon: Circle, color: 'text-gray-400 dark:text-slate-500', iconBg: 'bg-gray-100 dark:bg-neutral-800', label: stat.critic_id }
  const stage = CRITIC_STAGE[stat.critic_id] ?? '—'
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-700 shadow-sm dark:shadow-none flex flex-col w-full overflow-hidden">
      ...
      <div className="m-3 rounded-xl bg-stone-100 dark:bg-neutral-800 border border-gray-100 dark:border-neutral-800 px-4 pt-4 pb-4 flex flex-col gap-3">
        ...
        {/* Avatar + critic name + approval badge */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <CriticAvatar icon={meta.icon} iconBg={meta.iconBg} color={meta.color} />
            <h2 className="text-base font-semibold text-gray-900 dark:text-slate-100 tracking-tight">{meta.label}</h2>
          </div>
          <ApprovalBadge pct={stat.approval_pct} />
        </div>

        {/* Description */}
        <p className="text-sm text-gray-500 dark:text-slate-400 leading-relaxed">
          Scoring verdicts for the{' '}
          <span className="font-medium text-gray-700 dark:text-slate-300">{stage.toLowerCase()}</span>.
          Tracks approval rates, source confidence, and training signal quality.
        </p>
        ...
        {/* Total verdicts */}
        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-600 dark:text-slate-400">Total verdicts</span>
          <span className="text-sm font-semibold text-gray-900 dark:text-slate-100">{stat.total.toLocaleString()}</span>
        </div>

        {/* Avg score */}
        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-600 dark:text-slate-400">Avg score</span>
          <span className="text-sm font-semibold text-gray-900 dark:text-slate-100">{stat.avg_score}</span>
        </div>

        {/* Approval pill */}
        <ApprovalPill pct={stat.approval_pct} />

        {/* Training eligible */}
        <div className="flex items-start justify-between gap-4">
          <span className="text-sm text-slate-500 dark:text-slate-400 flex-shrink-0">Training eligible</span>
          <span className="text-sm font-medium text-gray-900 dark:text-slate-100 text-right">
            {stat.training_eligible.toLocaleString()}
          </span>
        </div>

        {/* Source breakdown */}
        <div className="flex items-center justify-between gap-4">
          <span className="text-sm text-slate-500 dark:text-slate-400 flex-shrink-0">Source</span>
          <span className="flex items-center gap-1.5 text-xs font-semibold text-gray-900 dark:text-slate-100">
            Claude {stat.source.claude}
            {stat.source.llm > 0 && (
              <>
                <span className="text-slate-300 dark:text-slate-600">·</span>
                <span className="flex items-center gap-1 text-blue-600 dark:text-blue-400">
                  <OllamaIcon size={13} />
                  Ollama MBody Model {stat.source.llm} ({stat.source.llm_pct}%)
                </span>
              </>
            )}
          </span>
      ...
      <div className="px-5 pb-4 pt-1 flex items-center justify-between">
        <span className="text-xs text-slate-500 dark:text-slate-400">
          {stat.training_eligible.toLocaleString()} training-eligible
        </span>
        {stat.override_rate_pct > 0 && (
          <span className="flex items-center gap-1 text-xs text-amber-500 dark:text-amber-400 font-medium">
            <AlertTriangle size={11} />
            {stat.override_rate_pct}% overridden
          </span>
        )}
      </div>
