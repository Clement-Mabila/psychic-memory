...
// ── Avatar circle (mirrors AgentAvatar) ──────────────────────────────────────
function CriticAvatar({ icon: Icon, iconBg, color }: { icon: any; iconBg: string; color: string }) {
  return (
...
// ── Ready / Accumulating badge (mirrors StatusBadge) ─────────────────────────
function ReadinessBadge({ ready }: { ready: boolean }) {
  return (
    <span className="flex items-center gap-1.5 text-sm font-medium">
      <span className={cn('w-2 h-2 rounded-full', ready ? 'bg-emerald-50 dark:bg-emerald-950/400' : 'bg-gray-300')} />
      <span className={ready ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 dark:text-slate-500'}>
...
// ── Verdict progress pill (replaces the "action detail" white pill) ───────────
function VerdictPill({
  ...
  return (
    <div className="rounded-lg bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 px-3 py-2.5 flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        <span className="text-xs font-normal text-stone-600 dark:text-slate-400">
          {exportable.toLocaleString()} verdicts available, awaiting remaining {(threshold - exportable).toLocaleString()}
        </span>
        <span className="text-xs font-semibold text-gray-400 dark:text-slate-500">{pct.toFixed(0)}%</span>
      </div>
...
// ── Main card ─────────────────────────────────────────────────────────────────
export function ReadinessCard({ critic }: Props) {
  const meta     = AGENT_META[critic.critic_id] ?? { icon: Circle, color: 'text-gray-400 dark:text-slate-500', iconBg: 'bg-gray-100 dark:bg-neutral-800', label: critic.critic_id }
  const pct      = Math.min(critic.pct_ready, 100)
  const barColor = pct >= 100 ? 'bg-emerald-400' : pct >= 60 ? 'bg-amber-400' : 'bg-sky-50 dark:bg-sky-950/400'
  const stage    = CRITIC_STAGE[critic.critic_id] ?? '—'
  const hasModel = !!critic.active_model
  const lastTrained = critic.days_since_training != null ? `${critic.days_since_training}d ago` : 'Never'

  const modelLabel = hasModel
    ? `${critic.active_model.version_tag}${critic.active_model.eval_accuracy != null ? ` · ${(critic.active_model.eval_accuracy * 100).toFixed(0)}%` : ''}`
    : '—'
  ...
  return (
    ...
    <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-700 shadow-sm dark:shadow-none flex flex-col w-full overflow-hidden">
      ...
      {/* ── Inner gray card — everything lives here ── */}
      <div className="m-3 rounded-xl bg-stone-100 dark:bg-neutral-800 border border-gray-100 dark:border-neutral-800 px-4 pt-4 pb-4 flex flex-col gap-3">

        {/* Icon + label + ready badge */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <CriticAvatar icon={meta.icon} iconBg={meta.iconBg} color={meta.color} />
            <h2 className="text-base font-semibold text-gray-900 dark:text-slate-100 tracking-tight">
              {meta.label}
            </h2>
          </div>
          <ReadinessBadge ready={critic.ready} />
        </div>

        {/* Stage description (replaces long description text) */}
        <p className="text-sm text-gray-500 dark:text-slate-400 leading-relaxed">
          Monitors the <span className="font-medium text-gray-700 dark:text-slate-300">{stage.toLowerCase()}</span> and
          scores each record using trained verdicts. Routing switches to the active ML model
          once readiness is reached.
        </p>
        ...
        {/* Last trained row */}
        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-600 dark:text-slate-400">Last trained</span>
          <span className="text-sm font-semibold text-gray-900 dark:text-slate-100">{lastTrained}</span>
        </div>

        {/* Verdict progress pill — white bg inside the gray card */}
        <VerdictPill
          exportable={critic.exportable_verdicts}
        ...
        {/* Active model */}
        <div className="flex items-start justify-between gap-4">
          <span className="text-sm text-slate-500 dark:text-slate-400 flex-shrink-0">Active model</span>
          <span className="text-sm font-medium text-gray-900 dark:text-slate-100 text-right">{modelLabel}</span>
        </div>

      </div>

      {/* ── Footer on white — LLM routing status + confidence threshold ── */}
      <div className="px-5 pb-4 pt-1 flex items-center justify-between">
        <button className="flex items-center gap-2 text-xs font-normal text-gray-800 dark:text-slate-200 hover:text-gray-500 dark:text-slate-400 transition-colors">
          <span className={critic.llm_routing_enabled ? 'text-blue-500 dark:text-blue-400' : 'text-gray-400 dark:text-slate-500'}>
            <OllamaIcon size={17} />
          </span>
          {critic.llm_routing_enabled ? 'Ollama routing on' : 'Claude only'}
        </button>

        {critic.confidence_threshold != null && (
          <span className="text-xs text-slate-600 dark:text-slate-400">
            {(critic.confidence_threshold * 100).toFixed(0)}% threshold
          </span>
        )}
      </div>
