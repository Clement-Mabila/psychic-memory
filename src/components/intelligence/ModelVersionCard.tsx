...
function OllamaIcon({ size = 16 }: { size?: number }) {
  return (
...
import { ArrowUpCircle, ArchiveX, AlignHorizontalJustifyCenter, Pencil, Check } from 'lucide-react'
...
const ICON_BG: Record<string, string> = {
  promoted:   'bg-emerald-100 text-emerald-600 dark:text-emerald-400',
  trained:    'bg-orange-100 text-orange-600',
  evaluating: 'bg-amber-100 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400',
  retired:    'bg-slate-100 dark:bg-neutral-800 text-slate-400 dark:text-slate-500',
  base:       'bg-slate-100 dark:bg-neutral-800 text-slate-400 dark:text-slate-500',
}
...
interface Props {
  ...
  onUpdate:      (id: number, patch: { base_model: string }) => void
  promoting:     boolean
  retiring:      boolean
  evaluating:    boolean
...
export function ModelVersionCard({ mv, onPromote, onRetire, onEvaluate, onUpdate, promoting, retiring, evaluating, isRunningEval }: Props) {
  const evalActive = isRunningEval || mv.status === 'evaluating'
  const iconCls = evalActive ? 'bg-amber-100 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400' : (ICON_BG[mv.status] ?? 'bg-slate-100 dark:bg-neutral-800 text-slate-400 dark:text-slate-500')
  const dotCls  = evalActive ? 'bg-amber-400 animate-pulse' : (STATUS_DOT[mv.status] ?? 'bg-slate-300')

  const [editingBase, setEditingBase] = useState(false)
  const [baseModel,   setBaseModel]   = useState<string>(mv.base_model ?? '')

  const scope   = mv.critic_id ? mv.critic_id.replace('_critic', '') : 'shared'
  const details = [
    mv.dataset?.example_count ? `${mv.dataset.example_count} examples` : null,
  ].filter(Boolean).join(' · ')

  function saveBase() {
    onUpdate(mv.id, { base_model: baseModel })
  ...
  return (
    <div className="flex items-start gap-4 px-5 py-3 bg-neutral-200 dark:bg-neutral-800 rounded-2xl shadow-sm dark:shadow-none hover:shadow-md dark:hover:shadow-none transition-shadow">
      ...
      {/* Icon */}
      <div className={cn('w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5', iconCls)}>
        <OllamaIcon size={22} />
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-0.5">
          <span className="text-sm font-semibold text-slate-800 dark:text-slate-200">{mv.version_tag}</span>
          <span className="text-xs text-slate-500 dark:text-slate-400">{formatRelativeTime(mv.created_at)}</span>
          {mv.is_active && (
          ...
          {evalActive && (
            <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 animate-pulse">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-50 dark:bg-amber-950/400 animate-ping inline-block" />
              Evaluating…
            </span>
          )}
        </div>

        {/* base_model — inline editable */}
          ...
          {editingBase ? (
            <>
              <input
                ...
                autoFocus
                className="text-xs border border-gray-300 dark:border-neutral-600 rounded px-1.5 py-0.5 outline-none focus:border-blue-400 dark:focus:border-blue-500 bg-white dark:bg-neutral-900 w-56"
              />
              <button onClick={saveBase} className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:text-emerald-400">
                <Check size={12} />
              </button>
            </>
          ) : (
            <>
              <span className="text-xs text-slate-600 dark:text-slate-400">{baseModel || '—'}</span>
              <button onClick={() => setEditingBase(true)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-400 transition-colors">
                <Pencil size={10} />
              </button>
            </>
          )}
          <span className="text-xs text-slate-400 dark:text-slate-500">· {scope}</span>
          {details && <span className="text-xs text-slate-400 dark:text-slate-500">· {details}</span>}
        </div>

        {mv.eval_accuracy != null && (
          <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
            {(mv.eval_accuracy * 100).toFixed(1)}% accuracy
            {mv.promoted_at && (
              <span className="font-normal text-slate-500 dark:text-slate-400 ml-1">
                · promoted {formatRelativeTime(mv.promoted_at)}
              </span>
            )}
          </p>
        )}
      ...
      <div className="flex items-center gap-2 flex-shrink-0 self-center">
        <span className={cn(
          'text-xs font-semibold px-2 py-1 rounded-2xl',
          MV_STATUS_STYLE[mv.status] ?? 'bg-slate-100 dark:bg-neutral-800 text-slate-500 dark:text-slate-400',
        )}>
          {mv.status}
        </span>

        {(mv.status === 'trained' || mv.status === 'retired') && (
          <button
            disabled={evaluating}
            onClick={() => onEvaluate(mv.id)}
            className="text-xs font-semibold px-2 py-1 rounded-2xl bg-blue-50 dark:bg-blue-950/40 text-white hover:bg-blue-600 transition-colors disabled:opacity-50"
          >
        ...
        {mv.status !== 'promoted' && mv.status !== 'base' && (
          <button
            ...
            className="text-xs font-semibold px-2 py-1 rounded-2xl bg-stone-400 text-white hover:bg-stone-50 dark:bg-neutral-800 transition-colors disabled:opacity-50"
          >
            <span className="flex items-center gap-1">
