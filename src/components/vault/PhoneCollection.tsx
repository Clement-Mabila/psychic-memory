...
import {
  ChevronLeft, ChevronRight, Search, XCircle, Phone, CheckCircle,
} from 'lucide-react'
...
export function PhoneCollection({ onBack, onSelectIdentity }: PhoneCollectionProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['vault-phones', page, search, tierFilter],
    queryFn:  () => vaultApi.getPhoneProfiles({
      page,
    ...
    <div className="bg-white dark:bg-zinc-950 min-h-screen">
      ...
      <div className="border-b border-zinc-200 dark:border-white/10 px-6 sm:px-8 pt-8 pb-6">
        ...
        <div className="flex items-start gap-4">
          <div className="w-14 h-14 rounded-xl flex items-center justify-center text-white font-bold text-lg shrink-0 bg-emerald-600">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400 max-w-xl">
              Every identity in the vault with a verified direct or mobile number — formatted and ready for outreach.
            </p>
            <div className="mt-2 flex flex-wrap gap-4 text-xs text-zinc-400 dark:text-zinc-500">
              <span className="flex items-center gap-1">
                <Phone className="w-3.5 h-3.5" />{total.toLocaleString()} contacts with phones
              </span>
            </div>
          </div>
        </div>
            ...
            {TIER_PILLS.map(p => (
              <button
                ...
                className={cn(
                  ...
                  tierFilter === p.id
                    ? 'bg-emerald-600/10 dark:bg-emerald-600/20 border-emerald-500/50 text-emerald-600 dark:text-emerald-300'
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-zinc-100 dark:border-white/10 bg-zinc-50 dark:bg-white/5">
                {['Name', 'Phone', 'Current Company', 'Title', 'Signal Tier', 'Appearances'].map(h => (
                  <th
                    key={h}
              ...
              {profiles.map((p: any) => {
                ...
                return (
                  <tr
                    ...
                    className="border-b border-zinc-50 dark:border-white/5 hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors cursor-pointer"
                  >
